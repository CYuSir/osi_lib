...
void *list_front(const list_t *list) {
    ...
    if (list && list->head) return list->head->data;
    return NULL;
}
...
void *list_back(const list_t *list) {
    assert(list != NULL);
    assert(!list_is_empty(list));
    if (list && list->tail) return list->tail->data;
    return NULL;
}
...
bool list_insert_after(list_t *list, list_node_t *prev_node, void *data) {
    ...
    assert(data != NULL);
    if (!list || !prev_node) return false;
...
bool list_remove(list_t *list, void *data) {
    ...
    if (!list || list_is_empty(list)) return false;
...
void list_clear(list_t *list) {
    assert(list != NULL);
    if (list)
        for (list_node_t *node = list->head; node;) node = list_free_node_(list, node);
    list->head = NULL;
    list->tail = NULL;
    list->length = 0;
}
