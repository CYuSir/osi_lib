...
// normal interface.
hash_map_t *hash_map_new_internal(size_t size, hash_index_fn hash_fn, key_free_fn key_fn, data_free_fn,
...
static const allocator_t untracked_calloc_allocator = {untracked_calloc, free};
...
void allocation_tracker_init(void) {
    ...
    allocations = hash_map_new_internal(allocation_hash_map_size, hash_function_pointer, NULL, free, NULL,
...
size_t allocation_tracker_resize_for_canary(size_t size) { return (!allocations) ? size : size + (2 * canary_size); }

static bool allocation_entry_freed_checker(hash_map_entry_t *entry, void *context) {
    allocation_t *allocation = (allocation_t *)entry->data;
    if (!allocation->freed) {
        *((size_t *)context) += allocation->size;  // Report back the unfreed byte count
        LOG_ERROR("%s found unfreed allocation. address: 0x%zx size: %zd bytes", __func__, (uintptr_t)allocation->ptr,
                  allocation->size);
    }
...
static void *untracked_calloc(size_t size) { return calloc(size, 1); }
