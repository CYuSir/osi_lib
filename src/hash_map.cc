...
hash_map_t *hash_map_new_internal(size_t num_bucket, hash_index_fn hash_fn, key_free_fn key_fn, data_free_fn data_fn,
                                  key_equality_fn equality_fn, const allocator_t *zeroed_allocator) {
    assert(hash_fn != NULL);
    assert(num_bucket > 0);
    ...
    hash_map->bucket =
        static_cast<hash_map_bucket_t *>(zeroed_allocator->alloc(sizeof(hash_map_bucket_t) * num_bucket));
...
hash_map_t *hash_map_new(size_t num_bucket, hash_index_fn hash_fn, key_free_fn key_fn, data_free_fn data_fn,
...
bool hash_map_set(hash_map_t *hash_map, const void *key, void *data) {
    ...
    if (hash_map->bucket[hash_key].list == NULL) {
        ...
        if (hash_map->bucket[hash_key].list == NULL) return false;
    }
    list_t *hash_bucket_list = hash_map->bucket[hash_key].list;
    ...
    if (hash_map_entry == NULL) return false;
...
void *hash_map_get(const hash_map_t *hash_map, const void *key) {
    ...
    hash_map_entry_t *hash_map_entry = find_bucket_entry_(hash_bucket_list, key);
    if (hash_map_entry != NULL) return hash_map_entry->data;
...
void hash_map_clear(hash_map_t *hash_map) {
    ...
    for (hash_index_t i = 0; i < hash_map->num_bucket; i++) {
        if (hash_map->bucket[i].list == NULL) continue;
...
void hash_map_foreach(hash_map_t *hash_map, hash_map_iter_cb callback, void *context) {
    ...
    for (hash_index_t i = 0; i < hash_map->num_bucket; ++i) {
        if (hash_map->bucket[i].list == NULL) continue;
        for (const list_node_t *iter = list_begin(hash_map->bucket[i].list); iter != list_end(hash_map->bucket[i].list);
             iter = list_next(iter)) {
            hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)list_node(iter);
            if (!callback(hash_map_entry, context)) return;
        }
    }
}
...
static void bucket_free_(void *data) {
    ...
    if (hash_map->key_fn) hash_map->key_fn((void *)hash_map_entry->key);
    if (hash_map->data_fn) hash_map->data_fn(hash_map_entry->data);
    hash_map->allocator->free(hash_map_entry);
}

static hash_map_entry_t *find_bucket_entry_(list_t *hash_bucket_list, const void *key) {
    if (hash_bucket_list == NULL) return NULL;

    for (const list_node_t *iter = list_begin(hash_bucket_list); iter != list_end(hash_bucket_list);
         iter = list_next(iter)) {
        hash_map_entry_t *hash_map_entry = (hash_map_entry_t *)list_node(iter);
...
static bool default_key_equality(const void *x, const void *y) { return x == y; }
