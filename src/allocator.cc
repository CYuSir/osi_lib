...
char *osi_strdup(const char *str) {
    ...
    char *new_string =
        static_cast<char *>(allocation_tracker_notify_alloc(alloc_allocator_id, malloc(real_size), size));
...
void *osi_malloc(size_t size) {
    size_t real_size = allocation_tracker_resize_for_canary(size);
    return allocation_tracker_notify_alloc(alloc_allocator_id, malloc(real_size), size);
}

void *osi_calloc(size_t size) {
    size_t real_size = allocation_tracker_resize_for_canary(size);
    return allocation_tracker_notify_alloc(alloc_allocator_id, calloc(1, real_size), size);
}
