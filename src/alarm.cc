...
alarm_t *alarm_new(void) {
    ...
    if (!alarms && !lazy_initialize()) return NULL;
    int error;
...
period_ms_t alarm_get_remaining_ms(const alarm_t *alarm) {
    ...
    if (alarm->deadline) remaining_ms = alarm->deadline - now();
...
static void schedule_next_instance(alarm_t *alarm, bool force_reschedule) {
    ...
    bool needs_reschedule = (!list_is_empty(alarms) && list_front(alarms) == alarm);
    if (alarm->callback) list_remove(alarms, alarm);
...
static void callback_dispatch(UNUSED_ATTR void *context) {
    while (true) {
        ...
        if (!callback_thread_active) break;
        ...
        // nothing left to do.
        if (list_is_empty(alarms) || (alarm = static_cast<alarm_t *>(list_front(alarms)))->deadline > now()) {
            reschedule_root_alarm();
            pthread_mutex_unlock(&monitor);
            continue;
        }
